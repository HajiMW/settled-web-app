...
import type { Card as CardType, FilmMetadata } from '@/types';
...
/**
 * Film card with poster image, title, year, director, genre badges, rating stars, streaming platform icons.
 */
export function FilmCard({ card }: FilmCardProps) {
  const metadata = card.metadata as FilmMetadata;
  ...
  return (
    <div className="w-full max-w-sm mx-auto">
      <div className="bg-white dark:bg-deep-navy/80 rounded-xl shadow-lg overflow-hidden">
        <div className="h-64 bg-gradient-to-br from-deep-navy/20 to-purple/20 flex items-center justify-center">
        ...
        <div className="p-5">
          ...
          <div className="flex flex-wrap gap-1.5 mt-3">
            {metadata.genre.map((genre) => (
              <Badge key={genre} variant="purple" size="sm">
                {genre}
              </Badge>
            ))}
          </div>
          <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-100 dark:border-gray-800">
            <span className="text-xs text-gray-500 dark:text-gray-400">Available on:</span>
